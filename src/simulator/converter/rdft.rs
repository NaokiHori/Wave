...
impl RdftPlan {
    pub fn new(nitems: usize) -> Result<Self, String> {
        ...
        for k in 0..nitems / 2 {
            let phase: f64 = 2f64 * PI * k as f64 / nitems as f64;
            table.push(Complex {
                real: phase.cos(),
                imag: phase.sin(),
            });
    ...
    pub fn exec_f(&mut self, xs: &mut [f64]) {
        ...
        for k in 1..nitems_sub {
            ...
            let z0 = Complex {
                real: 0.5f64 * zs[2 * k],
                imag: 0.5f64 * zs[2 * k + 1],
            };
            // 1 / 2 Z_{N / 2 - k}
            let z1 = Complex {
                real: 0.5f64 * zs[2 * (nitems_sub - k)],
                imag: 0.5f64 * zs[2 * (nitems_sub - k) + 1],
            };
    ...
    pub fn exec_b(&mut self, xs: &mut [f64]) {
        ...
        (zs[0], zs[1]) = {
            let x0: f64 = 0.5f64 * xs[0];
            let x1: f64 = 0.5f64 * xs[nitems_sub];
            (x0 + x1, x0 - x1)
        };
        for k in 1..nitems_sub {
            ...
            // 1/2 X_k
            let x0 = Complex {
                real: 0f64 + 0.5f64 * xs[k],
                imag: 0f64 - 0.5f64 * xs[nitems - k],
            };
            // 1/2 X_{k + N / 2}
            let x1 = Complex {
                real: 0f64 + 0.5f64 * xs[nitems_sub - k],
                imag: 0f64 + 0.5f64 * xs[nitems_sub + k],
            };
...
fn dft(nitems: usize, stride: usize, trigs: &[Complex], xs: &[f64], ys: &mut [f64]) {
    if 1 == nitems {
        ys[0] = xs[0];
        ys[1] = xs[1];
    } else if 0 == nitems % 2 {
        ...
        // naive O(N^2) DFT
        for k in 0..nitems {
            let y_ref: &mut [f64] = &mut ys[2 * k..];
            y_ref[0] = 0f64;
            y_ref[1] = 0f64;
            for n in 0..nitems {
                let arg: f64 = -2f64 * PI * n as f64 * k as f64 / nitems as f64;
                let twiddle = Complex {
                    real: arg.cos(),
                    imag: arg.sin(),
                };
...
fn idft(nitems: usize, stride: usize, trigs: &[Complex], xs: &[f64], ys: &mut [f64]) {
    if 1 == nitems {
        ys[0] = xs[0];
        ys[1] = xs[1];
    } else if 0 == nitems % 2 {
        ...
        for k in 0..nitems_sub {
            ...
            let e: Complex = Complex {
                real: 0.5f64 * ys[index_0],
                imag: 0.5f64 * ys[index_0 + 1],
            };
            let o: Complex = Complex {
                real: 0.5f64 * ys[index_1],
                imag: 0.5f64 * ys[index_1 + 1],
            };
            let o: Complex = o * twiddle;
            let y0: Complex = e + o;
            let y1: Complex = e - o;
        ...
        for k in 0..nitems {
            ...
            ys[index] /= nitems as f64;
            ys[index + 1] /= nitems as f64;
        }
    }
}
...
mod test {
    ...
    fn idft(nitems: usize, xs: &[Complex], ys: &mut [Complex]) {
        for n in 0..nitems {
            ...
            for k in 0..nitems {
                let arg: f64 = 2f64 * PI * k as f64 * n as f64 / nitems as f64;
                let twiddle = Complex {
                    real: arg.cos(),
                    imag: arg.sin(),
                };
    ...
    fn f() {
        ...
        for nitems in nitemss {
            ...
            let input: Vec<f64> = {
                ...
                (0..nitems).map(|_| rng.gen::<f64>() - 0.5f64).collect()
            };
            ...
            let output1: Vec<f64> = {
                let mut output: Vec<f64> = vec![0f64; nitems];
            ...
            // check L^1 norm
            let dif: f64 = {
                let mut dif: f64 = 0f64;
                for n in 0..nitems / 2 + 1 {
                    dif += (output0[n].real - output1[n]).abs() / nitems as f64;
                }
                for n in nitems / 2 + 1..nitems {
                    dif += (output0[n].imag - output1[n]).abs() / nitems as f64;
                }
                dif
            };
            assert!(dif < 1e-14f64 * nitems as f64);
        }
    }
    ...
    fn b() {
        ...
        for nitems in nitemss {
            ...
            let input: Vec<f64> = {
                ...
                let mut rng = rand::thread_rng();
                let seq: Vec<f64> = (0..nitems).map(|_| rng.gen::<f64>() - 0.5f64).collect();
                seq
            };
